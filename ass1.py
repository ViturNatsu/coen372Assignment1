...
plt.savefig('penguin-base-dt.png')
...
plt.savefig('penguin-top-dt.png')

grid_search_abalone_dt = GridSearchCV(DecisionTreeClassifier(), param_grid, scoring='accuracy', cv=5)
...
grid_search_penguin_mlp = GridSearchCV(MLPClassifier(), param_grid_mlp, scoring='accuracy', cv=5)
grid_search_penguin_mlp.fit(penguin_X_train, penguin_y_train)
best_penguin_mlp = grid_search_penguin_mlp.best_estimator_
...
for i in range(5):
    ...
    save_model_performance(best_penguin_dt, penguin_X_test, penguin_y_test, 'Penguin', 'Top-DT',
                           grid_search_penguin_dt.best_params_, penguin_performance_file)
    save_model_performance(penguin_base_mlp, penguin_X_test, penguin_y_test, 'Penguin', 'Base-MLP', 'Default',
                           penguin_performance_file)
    save_model_performance(best_penguin_mlp, penguin_X_test, penguin_y_test, 'Penguin', 'Top-MLP',
                           grid_search_penguin_mlp.best_params_, penguin_performance_file)

    save_model_performance(abalone_base_dt, abalone_X_test, abalone_y_test, 'Abalone', 'Base-DT', 'Default',
                           abalone_performance_file)
    save_model_performance(best_abalone_dt, abalone_X_test, abalone_y_test, 'Abalone', 'Top-DT',
                           grid_search_abalone_dt.best_params_, abalone_performance_file)
    save_model_performance(abalone_base_mlp, abalone_X_test, abalone_y_test, 'Abalone', 'Base-MLP', 'Default',
                           abalone_performance_file)
